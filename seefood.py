...
def label_image(img, confidence=50):
    ...
    # load the image from the URL or filename
    imgbytes = get_image(img)
    pillow_img = get_pillow_img(imgbytes)

    # call the detect_labels method to analyze the image
    response = rekognition.detect_labels(Image={'Bytes': imgbytes}, MinConfidence=confidence)

    # check if image contains hot dog
    hot_dog_found = False
    food_found = False
    ...
    # add label
    labeled_img = add_text_to_img(pillow_img, label_text, pos=(250, 50), color=(0, 0, 0), bgcolor=bg)
